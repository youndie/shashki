 ...
 *
 * **This is the fourth copy of eleven lines, and the count is the finding.** kvadrant-ui has it and keeps
 * it `internal` in `desktopTest` (research §1.2b); `:shared-ui` copied it for the same reason and
 * made it `internal` too; `:rider` copied it from `:shared-ui` and `:driver` from `:rider`. Each copy was correct
 * and each was forced — a screenshot helper cannot live in `commonMain` without making a UI library depend on
